...
function applyLive(
  ...
  if (moved && (lastShown == null || due)) {
    engine.applyLiveBias('source-2', bias, ts, lastHourBias)
    return { grew: true }
  }
  engine.syncLiveClock(ts)
  return { grew: false }
...
export function useH1Radar(engine: TreeEngine, enabled: boolean) {
  ...
  useEffect(() => {
    ...
    const source = remote ? null : new EventSource('/h1-radar/stream')

    const handlePayload = (live: H4LiveSnapshot | null) => {
      if (!active || !live) return
      if (live.iso === 'test' || live.feed === 'test') return
      if (live.h1Bias == null || live.h1Bias === '') return
      const ts = live.ts || Date.now()
      ...
      if (bias != null) {
        ...
        const anchor = updateHourAnchor('living-tree-hour-h1', ts, bias)
        ...
        if (lastHourBias == null) {
          ...
          lastHourBias = engine.lastBiasFromArchive('source-2', live?.id ?? null)
    ...
    const onMessage = (ev: MessageEvent) => {
      const data = ev.data
      if (!data || data.source !== 'h1-radar-bridge') return
      if (data.live) handlePayload(data.live as H4LiveSnapshot)
    }
    window.addEventListener('message', onMessage)

    fetch('/h1-radar/latest')
      .then((r) => r.json())
      .then((live) => {
        if (live && live.h1Bias) handlePayload(live as H4LiveSnapshot)
      })
      .catch(() => {})
